...
// HACKS/HOTPATCHES BEGIN HERE
void terminal_write(char* b, int len) {
  Serial.print(b);
}

// HACKS/HOTPATCHES END HERE
...
int editorReadKey() {
  String key = Strfun::read_loopy(false, "");
  char key_char = key.charAt(0);
  int key_code = key_char;
...
void readFile(fs::FS &fs, const char * path) {
   ...
   while(file.available()) printf("%c", file.read());
}
...
void writeFile(fs::FS &fs, const char * path, const char * message) {
   ...
   if(file.print(message)) printf("− file written\n\r");
   else printf("− frite failed\n\r");
}

void deleteFile(fs::FS &fs, const char * path){
   printf("Deleting file: %s\r\n", path);
   if(fs.remove(path)) printf("− file deleted\n\r");
...
void KiloCommand::cmd(String argv[10]) {
  // init text editor
  initEditor();    
  
  editorOpen(SPIFFS, argv[1].c_str()); // you need to write it first via writeFile()
